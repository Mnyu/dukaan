...
cd "${THIS_DIR}" || exit
DUKAAN_ADMIN_DIST_DIR="${THIS_DIR}/../../dukaan-user-distribution/target/dukaan-user-distribution-${APP_VERSION}"
if [[ ! -d "${DUKAAN_ADMIN_DIST_DIR}" ]]; then
  echo "${DUKAAN_ADMIN_DIST_DIR} does not exist."
  echo "dukaan-user-distribution build must be executed before building this."
  echo "Failed."
  exit 1
fi
...
docker build . -t "dukaan-user:${DOCKER_IMAGE_TAG}"
